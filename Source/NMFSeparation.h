...
public:
	...
	* NMF identifies factors-many components which are clustered to form the sources.
	* The return value is a vector of numOfSources many sources. Each source is a pair of
	* vectors of samples.
	*/
	static vector<vector<vector<double>>>* separate(vector<double>* lSampleVector, vector<double>* rSampleVector, int numOfSources, int factors);
};